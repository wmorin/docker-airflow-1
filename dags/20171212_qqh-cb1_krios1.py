...
args = {
    ...
    'pixel_size': 1.08,
    'cs': 2.7,
    'convert_gainref': False,    # convert gainref file from dm4 to mrc 
    'apply_gainref': True,  
}
...
with DAG( os.path.splitext(os.path.basename(__file__))[0],
    ...
    sum = LSFOperator( task_id='sum',
        ...
        lsf_script="""
#BSUB -o {{ ti.xcom_pull( task_ids='stack_file' )[0] | replace( '.tif', '.job' ) }}
#BSUB -W 3
#BSUB -We 1
#BSUB -n 1
...
cd -- "$( dirname {{ ti.xcom_pull( task_ids='stack_file' )[0] }} )"
avgstack > {{ ti.xcom_pull( task_ids='stack_file' )[0] | replace( '.tif', '.log' ) }} <<-'__AVGSTACK_EOF__'
{{ ti.xcom_pull( task_ids='stack_file' )[0] }}
/tmp/{{ dag_run.conf['base'] }}_avg.mrcs
/
__AVGSTACK_EOF__
newstack -rot -270 \
    {{ ti.xcom_pull( task_ids='new_gainref_file')[-1] }} \
...
e2proc2d.py \
    /tmp/{{ dag_run.conf['base'] }}_avg_gainrefd.mrc  \
    {{ ti.xcom_pull( task_ids='stack_file' )[0] | replace( '.tif', '.jpg' ) }} \
    --process filter.lowpass.gauss:cutoff_freq=0.05
rm -f /tmp/{{ dag_run.conf['base'] }}_avg.mrcs /tmp/{{ dag_run.conf['base'] }}_gainref.mrc /tmp/{{ dag_run.conf['base'] }}_avg_gainrefd.mrc
""",
    )
    ...
    summed_preview = FileGlobSensor( task_id='summed_preview',
        filepath="{{ ti.xcom_pull( task_ids='stack_file' )[0] | replace( '.tif', '.jpg' ) }}",
        recursive=True,
        poke_interval=1,
    )
...
""",
    ...
    stack_file = FileGlobSensor( task_id='stack_file',
        filepath="{{ dag_run.conf['directory'] }}/**/{{ dag_run.conf['base'] }}*.tif",
        recursive=True,
        poke_interval=1,
    )
    ...
    if args['convert_gainref']:
        ...
        gainref_file = FileGlobSensor( task_id='gainref_file',
            filepath="{{ dag_run.conf['directory'] }}/**/CountRef*.dm4",
            recursive=True,
            poke_interval=1,
        )
...
dm2mrc \
    {{ ti.xcom_pull( task_ids='gainref_file' )[-1] }} \
    ...
    if args['apply_gainref']:
        ...
        new_gainref_file = FileGlobSensor( task_id='new_gainref_file',
            # filepath="{{ ti.xcom_pull( task_ids='gainref_file' )[-1] | replace( '.dm4', '.mrc' ) }}",
            filepath="{{ dag_run.conf['directory'] }}/**/CountRef*.mrc",
            recursive=True,
            poke_interval=1,
        )
...
#/afs/slac/package/spack/opt/spack/linux-rhel7-x86_64/gcc-4.8.5/cuda-8.0.61-4boigicgv475rgoirhsdyiwsdz5rgde7/samples/7_CUDALibraries/simpleCUFFT/simpleCUFFT
...
MotionCor2  \
    -InTiff   {{ ti.xcom_pull( task_ids='stack_file' )[0] }} \
    -Gain     {{ ti.xcom_pull( task_ids='new_gainref_file' )[-1] }} \
...
""",
    ...
    stack_file >> sum >> summed_preview >> previews
    sum >> influx_sum
